...
public class DB extends SQLiteOpenHelper {
    ...
    public boolean deleteTaskByName(String taskName) {
        ...
        // Fetch the ID of the matching task
        Cursor cursor = database.query("tasks", new String[]{"id"}, "task_name = ?",
                new String[]{taskName}, null, null, null, "1");

        if (cursor != null && cursor.moveToFirst()) {
            int idIndex = cursor.getColumnIndex("id");
            if (idIndex != -1) { // Check if 'id' column exists
